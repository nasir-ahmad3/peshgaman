...
class CreateComment(CanCreateMixin, CommentCreateMixin):
    ...
    def form_valid(self, form):
        ...
        # send email 
        curent_site = get_current_site(self.request)
        ...
        if author_email:
            email = EmailMessage(
                'دیدگاه جدید',
                'دیدگاه جدید برای مقاله <{}> که شما نویسنده آن استید ارسال گردیده است \n {}{}'.format(article, curent_site, reverse('blog:detail', kwargs = {'slug': article.slug})), to=[author_email],
        ...
        if user_email:
            email = EmailMessage(
                'در یافت دیدگاه', 
                'پیام شما در یافت گردید و به زودی به آن پاسخ داده خواهد شد ', 
                to=[user_email],
            ) 
            email.send()
        ...
        if parent_email:
            email = EmailMessage(
                'پاسخی به دیدگاه شما ثبت گردیده است', 
                'پایخی به دیدگاه شما در مقاله {} ثبت گردیده است برایم مشاهده بر روی لینک ذیل کلیک نمایید : '.format(reverse('blog:detail', kwargs = {'slug' : article.slug})), 
                to=[parent_email],
            )
            email.send()
